...
non-empty artifact ID and supported artifact digest. The subsequent receipt
binds those outputs to the run, source, universe, policies, runtime config,
stage, checkpoint digest, and unique artifact name. Local completion and atomic
rename alone are not durability claims.
...
- targeted ME-SR26/ME-SR28/provider-seam/screening/workflow suites: 102 passed;
- new ME-SR28 runtime suite: 20 passed;
- complete `source_refresh` suite: 174 passed;
- complete `run` suite: 216 passed;
- complete Market Engine suite: 1,924 passed with one documented baseline
  failure caused by the absent historical local DATA06 compact-checksum
  manifest;
- complete repository suite: 2,591 passed with the same single documented
  baseline failure;
