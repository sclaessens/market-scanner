...
```bash
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m market_engine.run.end_to_end_dry_run_command \
...
The implementation is connector-backed in this ChatGPT session. The exact local commands above are the commands to run from a local checkout because this session could not clone the GitHub repository over the network for local pytest and dry-run execution.
